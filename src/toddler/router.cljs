...
(defhook use-rendered? [id]
  (let [{:keys [pathname]} (use-location)
    ...
    (when on-path?
      (.setItem js/sessionStorage last-rendered-key [id pathname]))
    on-path?))
  ...
  (let [{:keys [go]} (hooks/use-context -navigation-)
    ...
    (if-some [url (component-path tree component)]
      (fn redirect
        ([] (redirect nil))
        ([params]
         (let [url (str url (when params (str "?" (clj->query params))))]
           (go url))))
      (fn [& _]
        ...
        (hooks/use-effect
          ...
          (when (= location url)
            (cond
              ...
              (some? best) (push (component-path tree (:id best)))
              :else nil)))))
    (children props)))
